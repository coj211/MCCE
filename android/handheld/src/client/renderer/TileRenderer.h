...
public:
	...
	bool tessellateFireInWorld(Tile* tt, int x, int y, int z);
	...
	// 模组自定义模型（ModTile 的部件列表 / 微方块的格内几何）：
	// 逐部件画可带旋转/平移的长方体；部件坐标用格内单位（0..1）。
	bool tesselateModelInWorld(Tile* tt, int x, int y, int z);
	// 画一个部件（供 tesselateModelInWorld 与动画方块通道共用）。
	void renderModelBox(const ModBlockPart& p, Tile* tt, float bx, float by, float bz, float brightness);
