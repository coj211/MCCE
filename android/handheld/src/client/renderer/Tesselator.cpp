...
// 0.8.1 背包渲染：从当前 override 模式的顶点数据构建 MeshBuffer（Touch::InventoryPane
// 在 beginOverride 中收集物品图标顶点，然后调用本方法生成批量 VBO）。
// 顶点格式 VertexDeclPTC: pos(3f) + uv(2f) + color(4B)，24 字节。
MeshBuffer Tesselator::end() {
	if (!tesselating || _voidBeginEnd || vertices <= 0)
		return MeshBuffer();
	tesselating = false;
	// 顶点格式必须活得比 MeshBuffer 久（MeshBuffer 只存指针），所以放静态常量。
	static const MeshBuffer::VertexFormat s_vf = [] {
		MeshBuffer::VertexFormat vf;
		vf.enableField(MeshBuffer::VertexFormat::FIELD0);  // pos  3f @ 0
		vf.enableField(MeshBuffer::VertexFormat::FIELD1);  // uv   2f @ 12
		vf.enableField(MeshBuffer::VertexFormat::FIELD2);  // color 4B @ 20
		return vf;
	}();
	MeshBuffer mb;
	mb.load(&s_vf, _varray, vertices, NULL, 0, 0, GL_TRIANGLES);
	vertices = 0;
	p = 0;
	count = 0;
	return mb;
...
void Tesselator::scale3d(float sx, float sy, float sz) {
	_sx *= sx;
	_sy *= sy;
	_sz *= sz;
}
...
void Tesselator::vertex( float x, float y, float z )
{
	...
	vertex.z = _sz * (z + zo);
	if (_tilted) {
		// 0.8.1 方块图标的 30° 前倾（绕 X 轴）
		const float s = 0.5f, c = 0.8660254f; // sin30, cos30
		float yy = vertex.y, zz = vertex.z;
		vertex.y = c * yy - s * zz;
		vertex.z = s * yy + c * zz;
	}
...
// 不改状态的顶点快照(endToCPU 会终止 tesselating,此函数不改变任何状态,须在 end() 前调用)
void Tesselator::snapshotTo(std::vector<uint8_t>& outData, int& outVertexCount)
{
	outVertexCount = vertices;
	if (p > 0) {
		int bytes = p * sizeof(VERTEX);
