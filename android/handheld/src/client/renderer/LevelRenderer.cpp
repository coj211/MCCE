	...
	destroyProgress(0)
{
	...
	// macOS/Linux and 256 for other platforms. Use the per-platform cap to size exactly.
	//
	// Android 必须走 256：桌面上限 1024 会让 numListsOrBuffers 达到 135200，
	// Adreno 等移动 GLES 驱动分配不了这么多 VBO，glGenBuffers 失败后绑定无效
	// buffer，顶点指针退化成客户端地址，glDrawArrays 直接 SIGSEGV。
	...
	// 值初始化：万一 glGenBuffers 失败，这里全为 0（绑定 0 = 解绑）而不是垃圾值。
	chunkBuffers = new GLuint[numListsOrBuffers]();
	glGenBuffers2(numListsOrBuffers, chunkBuffers);
	{
		GLenum _bufErr = glGetError();
		if (_bufErr)
			LOGI("[render] glGenBuffers(%d) failed: 0x%x\n", numListsOrBuffers, _bufErr);
	}
	LOGI("numBuffers: %d\n", numListsOrBuffers);
#else
