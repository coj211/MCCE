...
void Textures::unloadTexture(const std::string& resourceName)
{
	TextureMap::iterator it = idMap.find(resourceName);
	if (it != idMap.end()) {
		if (it->second != Textures::InvalidId) {
			glDeleteTextures(1, &it->second);
			// 同时清纹理数据缓存：glDeleteTextures 后 glGenTextures 会复用同一 id，
			// 而 loadedImages.insert 对已存在 key 不覆盖 → 重载后仍显示旧数据（世界截图不更新根因）
			loadedImages.erase(it->second);
		}
		idMap.erase(it);
	}
	lastBoundTexture = Textures::InvalidId;
...
bool Textures::replaceTextureData(const std::string& resourceName, int w, int h, const unsigned char* rgba)
{
	TextureMap::iterator it = idMap.find(resourceName);
	if (it == idMap.end() || it->second == Textures::InvalidId) return false;
	TextureImageMap::iterator di = loadedImages.find(it->second);
	if (di == loadedImages.end()) return false;
	TextureData& td = di->second;
	if (!td.memoryHandledExternally && td.data)
		delete[] td.data;
	td.w = w;
	td.h = h;
	td.numBytes = w * h * 4;
	...
	td.transparent = true;
	td.data = new unsigned char[td.numBytes];
	if (rgba)
		memcpy(td.data, rgba, td.numBytes);
	return true;
}
...
TextureData* Textures::loadAndGetTextureData( const std::string& resourceName ) {
	// 0.8.1 GUI 移植：加载并返回贴图数据
	TextureId id = loadTexture(resourceName);
	if (!isTextureIdValid(id))
		return NULL;
	return const_cast<TextureData*>(getTemporaryTextureData(id));
}
