...
// Other systems might run it, if they #define OPENGL_ES
	...
	#define GL_QUADS 0x0007
        ...
        #endif
        // GLES 1.1 只有 float 版本：glDepthRangef / glClearDepthf。
        // 桌面代码里的 glDepthRange / glClearDepth 在这里统一映射过去。
        #ifndef glDepthRange
            #define glDepthRange(n, f)   glDepthRangef(n, f)
        #endif
        #ifndef glClearDepth
            #define glClearDepth(x)      glClearDepthf(x)
        #endif
    #endif
#elif defined(MACOS)
    ...
    // Dedicated server: no graphics context at all, and no GL call is ever
    // made (Minecraft::update() 的渲染段在 STANDALONE_SERVER 下整段被排除)。
    //
    // 但共享头文件仍然大量引用 GL 的类型与常量（Textures.h 的 TextureId、
    // Tesselator.h 的 GLsizei、RenderChunk.h 的 GLuint、各处 GL_TEXTURE_2D /
    // GL_BLEND 常量……），所以这里仍然 include GL 头 —— 只取声明，不调函数。
    //
    // 这段只在 STANDALONE_SERVER 时生效，客户端构建走下面原有的 win32 分支，
    // 行为一字不变。
    #ifdef WIN32
	   #include <WinSock2.h>
	   #include <Windows.h>
...
// 共享头文件与共享代码里还会用到这些 GL 状态包装（glBindTexture2 / glEnable2 …）。
// 服务器既没有 GL 上下文、也永远走不到渲染路径，所以统一给成空操作：既让它们编得过，
// 又保证万一被调用也不会去碰未初始化的 GL 函数指针（那会直接崩）。
#define glBindTexture2(m, z)                ((void)0)
#define glEnable2(s)                        ((void)0)
#define glDisable2(s)                       ((void)0)
#define glColor4f2(r, g, b, a)              ((void)0)
#define glBlendFunc2(src, dst)              ((void)0)
#define glShadeModel2(s)                    ((void)0)
...
#define glTexParameteri2(m, o, v)           ((void)0)
#define glTexImage2D2(a,b,c,d,e,f,g,h,i)    ((void)0)
#define glTexSubImage2D2(a,b,c,d,e,f,g,h,i) ((void)0)
#define glGenBuffers2(s, id)                ((void)0)
#define glBindBuffer2(s, id)                ((void)0)
#define glBufferData2(a, b, c, d)           ((void)0)
#define anGenBuffers(n, buf)                ((void)0)
