...
public:
	...
	public:
		...
		// 移动版专属：用摇杆代替十字键（见 TouchscreenInput_TestFps）
		static const Option USE_TOUCH_JOYSTICK;
...
public:
	...
	bool serverVisible;
	bool isJoyTouchArea;
	// 移动版专属：开启后用摇杆（空心圆 + 可拖动实心圆）代替十字键
	bool useTouchJoystick;
	bool useTouchScreen;
    ...
    void toggle(const Option* option, int dir) {
		...
		if (option == &Option::USE_TOUCH_JOYSTICK) useTouchJoystick = !useTouchJoystick;
    ...
    bool getBooleanValue(const Option* item) {
		...
		if (item == &Option::USE_TOUCH_JOYSTICK)
			return useTouchJoystick;
