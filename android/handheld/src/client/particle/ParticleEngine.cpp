...
void ParticleEngine::addCustom(Particle* p) {
	_customParticles.push_back(p);
}
...
void ParticleEngine::tick() {
	...
	// Custom particles
	for (size_t i = 0; i < _customParticles.size(); ++i) {
		Particle* p = _customParticles[i];
		p->tick();
		if (p->removed) {
			_customParticles.erase(_customParticles.begin() + i);
			--i;
			delete p;
		}
	}
...
void ParticleEngine::render(Entity* player, float a) {
	// Entity rendering can leave GL_BLEND/GL_TEXTURE_2D disabled; particles
	// are translucent and need both, so force them on here.
	glEnable2(GL_BLEND);
    ...
    for (int tt = 0; tt < 3; tt++) {
        ...
        for (unsigned int i = 0; i < particles[tt].size(); i++) {
            ...
            p->render(t, a, xa, ya, za, xa2, za2, (tt == TERRAIN_TEXTURE) ? 32 : 16);
        }
        t.draw();
    }

	// Custom (JS-defined) particles: group by texture, one draw per group.
	if (!_customParticles.empty()) {
		glEnable2(GL_BLEND);
		glBlendFunc2(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
		std::map<unsigned int, std::vector<Particle*> > groups;
		for (size_t i = 0; i < _customParticles.size(); ++i)
			groups[((ScriptedParticle*)_customParticles[i])->getTextureId()].push_back(_customParticles[i]);
		Tesselator& ct = Tesselator::instance;
		for (std::map<unsigned int, std::vector<Particle*> >::iterator it = groups.begin(); it != groups.end(); ++it) {
			glBindTexture(GL_TEXTURE_2D, it->first);
			ct.begin();
			for (size_t i = 0; i < it->second.size(); ++i)
				it->second[i]->render(ct, a, xa, ya, za, xa2, za2, 16);
			ct.draw();
		}
	}
...
void ParticleEngine::renderLit(Entity* player, float a) {
    ...
    for (int i = 0; i < size; i++) {
        Particle* p = pl[i];
		p->render(t, a, xa, ya, za, xa2, za2, 16);
    }
}
