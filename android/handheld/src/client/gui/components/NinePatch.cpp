...
void NinePatchLayer::draw( Tesselator& t, float x, float y ) {
	// 09b · UI 覆盖：面板九宫格皮肤（mod UI.skin 9 张 png）。键由持有者设置
	// （如 PlayScreen 的 frame -> uiSkinKey="play.frame"）。若 mod 注册表里有
	// 该元素的皮肤，用 mod 九图重绘面板（角按图自身像素、边/中心拉伸）。
	if (!uiSkinKey.empty()) {
		extern bool modDrawNinePatchSkin(const std::string& screenKey,
			float x, float y, float w, float h);
		if (modDrawNinePatchSkin(uiSkinKey, x, y, w, h))
			return;
	}
