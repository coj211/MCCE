...
void InventoryPane::renderBatch( std::vector<GridItem>& items, float alpha )
{
	...
	// 0.6.1 触摸屏在渲染物品网格（含 3D 方块图标）前整体禁用深度测试：
	// renderToolBar/其它 GUI 元素会把 depth test 重新打开，而 3D 方块图标
	// (glPushMatrix z=-8) 的深度大于先画的 2D 面板面，会被深度测试挡掉。
	// 背包/物品栏里叶子、冰、mod 方块不显示即由此而来；2D 贴图不受影响。
