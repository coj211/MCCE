...
public:
	...
	// 09b · UI 覆盖：九宫格皮肤键（如 "play.frame"）。设置后 draw 时若 mod
	// 注册表里有该元素的 UI.skin（9 张 png），就改用 mod 九图绘制面板背景。
	void setUiSkinKey(const std::string& key) { uiSkinKey = key; }
...
private:
    ...
    Textures* textures;
	int excluded;
	std::string uiSkinKey;   // 09b: mod UI.skin key ("" = no skin lookup)
