...
void ImageButton::setupDefault() {
	...
	yOffset = 16; // 0.8.1 文字基线默认偏移（render 里文字画在 y+yOffset）
}
...
void ImageButton::render(Minecraft* minecraft, int xm, int ym) {
	...
	if (!active) {
		drawCenteredString(font, msg, x + width / 2, y + yOffset, 0xffa0a0a0);
	} else {
		if (hovered || selected) {
			drawCenteredString(font, msg, x + width / 2, y + yOffset + 1/*(h - 16)*/, 0xffffa0);
		} else {
			drawCenteredString(font, msg, x + width / 2, y + yOffset/*(h - 48)*/, 0xe0e0e0);
		}
	}
}
