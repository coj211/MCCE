...
static std::string sliderValueText(SliderType sliderType, const Options::Option* option,
	...
	if (sliderType == SliderStep) {
		if (option == &Options::Option::RENDER_DISTANCE) {
			...
			// Show the language's own name, not a translated label:
			// "English" always shows as English, 简体中文 as 简体中文.
			return curStepValue ? "简体中文" : "English";
		}
...
void Slider::render( Minecraft* minecraft, int xm, int ym ) {
	// Reserve right portion for value text (64px fits 4 CJK chars like 简体中文)
	const int textAreaW = 64;
