...
class CraftingCategoryButton: public ImageButton {
	typedef ImageButton super;
public:
	CraftingCategoryButton(int id, const ImageButton* const* selectedPtr, NinePatchLayer* stateNormal, NinePatchLayer* statePressed)
	:	super(id, ""),
...
void PaneCraftingScreen::initCategories() {
	...
	// Category buttons
	for (int i = 0; i < numCategories; ++i) {
		ImageButton* button = new CraftingCategoryButton(100 + i, &selectedCategoryButton, guiSlotCategory, guiSlotCategorySelected);
...
void PaneCraftingScreen::buttonClicked(Button* button) {
	...
	if (button->id >= 100 && button->id < 200) {
		...
		selectedCategoryButton = (CraftingCategoryButton*)button;
	}
}
