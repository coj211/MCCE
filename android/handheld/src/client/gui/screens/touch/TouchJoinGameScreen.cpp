...
:	bJoin(  2, I18n::get("menu.multiplayer")),
...
void JoinGameScreen::init()
{
	...
	::JoinGameScreen::loadCustomServers();
...
void JoinGameScreen::setupPositions() {
	...
	bHeader.width = width - bHeader.x - bAddServer.width - 8;
}
...
void JoinGameScreen::tick()
{
	...
	{
		const std::vector<::JoinGameScreen::CustomServer>& cs = ::JoinGameScreen::customServers();
...
void JoinGameScreen::render( int xm, int ym, float a )
{
	...
	if (hasNetwork) {
		std::string s = I18n::get("touchjoin.scanning");
		...
		drawCenteredString(minecraft->font, I18n::get("touchjoin.wifiDisabled"), baseX, 8, 0xffffffff);
	}
}
