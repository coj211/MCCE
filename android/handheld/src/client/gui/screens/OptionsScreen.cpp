...
void OptionsScreen::closeScreen() {
	...
	// 设置里能改「虚拟摇杆」「分离触屏控制」这类触摸控件外观，改完必须让
	// inputHolder 重跑 onConfigChanged 重建布局（reloadOptions 内部会 setSize），
	// 否则关掉虚拟摇杆后原先的十字键不会回来。save() 已在上一行，update() 读回的
	// 就是刚写下的新值。
...
void OptionsScreen::generateOptionScreens(void) {
	...
	this->optionPanes[1]->createOptionsGroup("options.group.input").addOptionItem(&Options::Option::SENSITIVITY, this->minecraft).addOptionItem(&Options::Option::INVERT_MOUSE, this->minecraft).addOptionItem(&Options::Option::LEFT_HANDED, this->minecraft).addOptionItem(&Options::Option::USE_TOUCHSCREEN, this->minecraft).addOptionItem(&Options::Option::USE_TOUCH_JOYPAD, this->minecraft).addOptionItem(&Options::Option::USE_TOUCH_JOYSTICK, this->minecraft).addOptionItem(&Options::Option::PIXELS_PER_MILLIMETER, this->minecraft).addOptionItem(&Options::Option::SPRINT, this->minecraft).addOptionItem(&Options::Option::AUTO_JUMP, this->minecraft).addOptionItem(&Options::Option::SWAP_JUMP_AND_SNEAK, this->minecraft);
