...
#include "screens/ScreenChooser.h"
#include "../Minecraft.h"
...
// UI 可用宽/高：扣掉“系统接管、不派发触摸”的右侧区域（当成屏幕外），
// 见 AppPlatform::getUiRightInset()。
int Gui::uiScreenWidth() const {
	int inset = 0;
	if (minecraft && minecraft->platform())
		inset = minecraft->platform()->getUiRightInset();
	int w = (int)((minecraft->width - inset) * InvGuiScale);
	return (w > 1) ? w : 1;
}

int Gui::uiScreenHeight() const {
...
void Gui::render(float a, bool mouseFree, int xMouse, int yMouse) {
	...
	const int screenWidth = uiScreenWidth();
	const int screenHeight = uiScreenHeight();
	blitOffset = -90;
	renderProgressIndicator(isTouchInterface, screenWidth, screenHeight, a);
	renderChatButton(screenWidth, screenHeight);
	...
	glEnable2(GL_BLEND);
	// 聊天屏（ChatInputScreen）会自己画一份可滚动的聊天记录；这里跳过 HUD 的
	// 那 10 行，免得两份聊天叠在一起（关掉聊天屏后行为与原来完全一致）。
	if (!(minecraft->screen && minecraft->screen->isChatScreen())) {
		unsigned int max = 10;
...
//        glEnable(GL_ALPHA_TEST);
	...
	glEnable2(GL_ALPHA_TEST);
#if defined(_WIN32) || defined(__ANDROID__)
	// Let mods draw their own HUD on top of everything.
	if (ModEngine::instance)
		ModEngine::instance->fireEvent("onGuiRender");
#endif
}

int Gui::getSlotIdAt(int x, int y) {
	int screenWidth = uiScreenWidth();
	int screenHeight = uiScreenHeight();
	x = (int)(x * InvGuiScale);
	y = (int)(y * InvGuiScale);
...
void Gui::renderChatButton(int screenWidth, int screenHeight) {
	// 照搬 Multiplatform-1.6.4_classic 的 HUD 聊天按钮
	// （modifiedeight/impl/input/TouchscreenInput.cpp 里的 chatButton）：
	//   图集 gui/gui.png（256×256），图标在 (200,82)、18×18，画进 26×26 触摸区；
	//   颜色 Color4(1,1,1,0.49804) → ARGB 0x7FFFFFFF（半透明白）；
	//   位置在右上角：x = width - (26+4)，y = 4。
	// UV 按图集尺寸归一，与 1.6.4 的 sub_D6604D0C()（像素×1/256）一致。
	const int bw  = 26;
	const int bh  = 26;
	const int bx0 = screenWidth - bw - 4;
	const int by0 = 4;

	if (minecraft->textures->loadAndBindTexture("gui/gui.png")) {
		TextureData* td = minecraft->textures->loadAndGetTextureData("gui/gui.png");
		if (td && td->w > 0 && td->h > 0) {
			const float iconU = 200.0f, iconV = 82.0f, iconSize = 18.0f;
			const float u0 = iconU / (float)td->w;
			const float v0 = iconV / (float)td->h;
			const float u1 = (iconU + iconSize) / (float)td->w;
			const float v1 = (iconV + iconSize) / (float)td->h;
			// 半透明必需：HUD 走到这一步时 GL_BLEND 还没开（官方在 Gui::render
			// 更靠后的位置才 glEnable2(GL_BLEND)，且不重设 blendFunc）。
			// 所以这里必须"借用后原样归还"：否则改动的 blend 开关/blendFunc 会
			// 泄漏给随后绘制的物品栏(renderToolBar)、聊天等，把它们也变透明。
			const GLboolean hadBlend = glIsEnabled(GL_BLEND);
			GLint prevSrc = GL_ONE, prevDst = GL_ZERO;
			...
			Tesselator& t = Tesselator::instance;
			t.begin();
			t.colorABGR(0x7fffffff);   // 白 + alpha 0.498（同 1.6.4 的 _D6E06660）
			t.vertexUV((float)bx0,        (float)(by0 + bh), blitOffset, u0, v1);
			t.vertexUV((float)(bx0 + bw), (float)(by0 + bh), blitOffset, u1, v1);
			t.vertexUV((float)(bx0 + bw), (float)by0,        blitOffset, u1, v0);
			t.vertexUV((float)bx0,        (float)by0,        blitOffset, u0, v0);
			t.draw();

			glBlendFunc2((GLenum)prevSrc, (GLenum)prevDst);
			if (!hadBlend)
				glDisable2(GL_BLEND);
		}
	}
...
void Gui::handleClick(int button, int x, int y) {
	...
	// Chat button (top-right corner, logical coords).
	{
		int lx = (int)(x * InvGuiScale);
		int ly = (int)(y * InvGuiScale);
		int screenWidth = uiScreenWidth();
		LOGI("[hudclick] handleClick %d,%d -> logical %d,%d, screenW=%d (chatBox x[%d..%d] y[4..30])\n",
		     x, y, lx, ly, screenWidth, screenWidth - 30, screenWidth - 4);
		if (lx >= screenWidth - 30 && lx <= screenWidth - 4 && ly >= 4 && ly <= 30) {
			LOGI("[hudclick] CHAT BUTTON HIT -> setScreen(SCREEN_CHAT)\n");
			minecraft->screenChooser.setScreen(SCREEN_CHAT);
			return;
		}
	}
...
void Gui::addMessage(const std::string& _string) {
	...
	// 上限：聊天界面要能往回翻历史（原来只留 30 条，翻不了几条）。
	while (guiMessages.size() > 200) {
	...
	for (unsigned int i = 0; i < guiMessages.size() && i < max; i++) {
		if (guiMessages.at(i).ticks < 20 * 10 || isChatting) {
			...
			if (alpha > 0) {
				...
				const float textW = font->width(msg);
				const float x = 2;   // left-aligned
				const float y = (float)(baseY - i * 12);
				this->fill(x - 2, y - 1, x + textW + 2, y + 11, (alpha / 2) << 24);
				glEnable(GL_BLEND);
...
// 0.8.1 背包移植：2 参重载
void Gui::renderToolBar( float a, int ySlot ) {
	renderToolBar(a, ySlot, uiScreenWidth());
}
