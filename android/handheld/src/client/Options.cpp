...
void Options::initDefaultValues() {
	...
	isLeftHanded = false;
	language = "zh_CN"; // 0.8.1 GUI 移植：默认中文

	isJoyTouchArea = false;
	useTouchJoystick = false;   // 移动版专属：默认还是十字键
	...
	classicGUI = true;   // 0.8.1 GUI 移植：默认经典样式
	neonColorTheme = 0;
	classicBackground = true; // 0.8.1 GUI 移植：默认 panorama 全景背景
	// 0.8.1 GUI 移植：设置页新增字段默认值
	showCoordinates = false;
	debugScreen = false;
	hudCameraButton = true;
	sprintEnabled = false;
	autoJump = true;
	swapJumpAndSneak = false;
	brightness = 0.5f;
	fogEnabled = true;
	fancySkies = true;
...
const Options::Option
	...
	Options::Option::LANGUAGE             (21, "options.language",          false, false),
	// 0.8.1 GUI 移植：设置页新增选项（序号 22+）
	Options::Option::NAME                 (22, "options.name",                false, false),
	Options::Option::SHOW_COORDINATES     (23, "options.showcoordinates",     false, true),
	Options::Option::DEBUG_SCREEN         (24, "options.debugscreen",         false, true),
	Options::Option::HUD_CAMERA_BUTTON    (25, "options.hudcamerabutton",     false, true),
	Options::Option::SPRINT               (26, "options.sprint",              false, true),
	Options::Option::AUTO_JUMP            (27, "options.autojump",            false, true),
	Options::Option::SWAP_JUMP_AND_SNEAK  (28, "options.swapjumpandsneak",    false, true),
	Options::Option::BRIGHTNESS           (29, "options.gamma",               true,  false),
	Options::Option::FOG_ENABLED          (30, "options.fogenabled",          false, true),
	Options::Option::FANCY_SKIES          (31, "options.fancyskies",          false, true),
	Options::Option::CLASSIC_TEXTURES     (32, "options.classictextures",     false, true),
	Options::Option::ANIMATE_TEXTURES     (33, "options.animatetextures",     false, true),
	Options::Option::ANIMATE_WATER        (34, "options.animatewater",        false, true),
	Options::Option::ANIMATE_LAVA         (35, "options.animatelava",         false, true),
	Options::Option::ANIMATE_FIRE         (36, "options.animatefire",         false, true),
	Options::Option::SMOOTH_CHUNKS        (37, "options.smoothchunks",        false, true),
	Options::Option::LOD_CHUNKS           (38, "options.lodchunks",           false, true),
	Options::Option::MARKETPLACE          (39, "options.marketplace",         false, true),
	Options::Option::SHOW_FPS             (40, "options.showfps",             false, true),
	Options::Option::DISCORD_RPC          (41, "options.discordrpc",          false, true),
	Options::Option::PANORAMA_ANGLE       (42, "options.panoramaAngle",       false, false),
	Options::Option::CHAT_COLOR           (43, "options.chatcolor",           false, false),
	Options::Option::CHAT_BG_COLOR        (44, "options.chatbgcolor",         false, false),
	Options::Option::CLASSIC_BACKGROUND   (45, "options.classicbackground",   false, true),
	Options::Option::CLASSIC_GUI          (46, "options.classicgui",          false, true),
	Options::Option::NEON_COLOR_THEME     (47, "options.neoncolortheme",      false, false),
	Options::Option::NEW_ADDITIONS        (48, "options.clientmode",         false, false),
	// 移动版专属：用摇杆代替十字键
	Options::Option::USE_TOUCH_JOYSTICK  (49, "options.touchjoystick",     false, true);
...
void Options::update()
{
	...
	for (unsigned int i = 0; i < optionStrings.size();) {
		...
		// 移动版专属：摇杆代替十字键（无触摸屏的平台强制关）
		if (key == OptionStrings::Controls_UseTouchJoystick) {
			readBool(value, useTouchJoystick);
			if (!minecraft->useTouchscreen())
				useTouchJoystick = false;
		}
		...
		if (key == OptionStrings::Game_Language) {
			readString(value, language);
			// 语言文件(data/lang/*.lang)可被玩家扩展；只校验该语言确实存在，
			// 不存在才回退默认。空值一律用默认中文。
			bool known = false;
			StringVector langs = I18n::availableLanguages(minecraft ? minecraft->platform() : NULL);
			for (size_t i = 0; i < langs.size(); ++i) {
				if (langs[i] == language) { known = true; break; }
			}
			if (!known)
				language = "zh_CN";
		}
...
void Options::save()
{
	...
	addOptionToSaveOutput(stringVec, OptionStrings::Game_Language, language);
	...
	addOptionToSaveOutput(stringVec, OptionStrings::Controls_UseTouchJoystick, useTouchJoystick);
...
bool Options::readString(const std::string& string, std::string& value) {
	value = Util::stringTrim(string);
	return !value.empty();
}
...
void Options::setLanguageValue( int idx ) {
	const char* languages[] = { "en_US", "zh_CN" };
	language = languages[idx & 1];
	I18n::loadLanguage(minecraft->platform(), language);
}

void Options::toggleLanguage() {
	const char* languages[] = { "en_US", "zh_CN" };
	int idx = (language == "zh_CN") ? 0 : 1;
	language = languages[idx];
	I18n::loadLanguage(minecraft->platform(), language);
}

void Options::getNeonColors(int& colorA, int& colorB) {
	// 0.8.1 GUI 移植：霓虹主题配色
	if (neonColorTheme == 1) {
		colorA = 0xFFFF0000; colorB = 0xFFFFD700;
	} else if (neonColorTheme == 2) {
		colorA = 0xFF00FF00; colorB = 0xFFCCFF00;
	} else if (neonColorTheme == 3) {
		colorA = 0xFFBD00FF; colorB = 0xFFFF00F0;
	} else if (neonColorTheme == 4) {
		colorA = 0xFFFF5F00; colorB = 0xFFFFD700;
	} else {
		colorA = 0xFF00FFFF; colorB = 0xFFFF00B4;
	}
}
