...
bool LocalPlayer::isSolidTile(int x, int y, int z) {
	...
	return tileId > 0 && Tile::tiles[tileId] != NULL && Tile::tiles[tileId]->material->isSolid();
}
...
void LocalPlayer::aiStep() {
	...
	if (abilities.mayfly) {
		...
		if (!wasJumping && input->jumping) {
			LOGI("[fly] jump edge: triggerTime=%d mayfly=%d flying=%d\n",
			     jumpTriggerTime, (int)abilities.mayfly, (int)abilities.flying);
			if (jumpTriggerTime <= 0) jumpTriggerTime = 5;   // 双击窗口（tick，20Hz → 0.25s）
			else {
				abilities.flying = !abilities.flying;
				jumpTriggerTime = 0;
				LOGI("[fly] TOGGLE -> flying=%d\n", (int)abilities.flying);
			}
		}
		if (abilities.flying) {
			if (input->wantUp) {
...
float LocalPlayer::getFieldOfViewModifier() {
	...
	if (sprinting) targetFov *= 1.25f;   // sprint: wider FOV
...
{
	// 世界规则「死亡不掉落」：打开时背包与盔甲原样保留，也不叫服务器去丢。
	...
	if (level->isClientSide) {
		SendInventoryPacket packet(this, !keepInventory);
		minecraft->raknetInstance->send(packet);
	}
	if (!keepInventory) {
