...
#include "../../../../mod/ModEngine.h"   // 模组 HUD 按钮占用指针（getCapturedPointer）
...
public:
	...
	// HUD 上的按钮（D-pad、跳跃、飞行升降、潜行）不能算进转向区：
	// 手指按住按钮不动（移动量小）时，上面的状态机会在 400ms 后判成
	// State_Destroy（= 开始破坏方块），于是「按着方向键 / 跳跃键」就把准星
	// （或手指）瞄准的方块挖了。这些矩形登记进来，onConfigChanged 时统一从
	// _area 里排除。矩形对象由调用方持有，这里只做值拷贝。
	void setControlAreas(const RectangleArea* areas, int count) {
		if (count < 0) count = 0;
		if (count > MaxControlAreas) count = MaxControlAreas;
		for (int i = 0; i < count; ++i)
			_ctrlArea[i] = areas[i];
		_ctrlAreaCount = count;
	}
	...
	virtual void onConfigChanged(const Config& c) {
		if (false && _options->isJoyTouchArea) {
			...
			// HUD 按钮区（由 IInputHolder 通过 setControlAreas 填进来）也要排除：
			// 否则按住跳跃键 / 飞行升降键会被当成「按住转向区不动」→ 开始挖方块。
			for (int i = 0; i < _ctrlAreaCount; ++i)
				_area.exclude(&_ctrlArea[i]);
#ifdef __APPLE__
...
//			const float MaxTurnY = 100.0f;
	...
	virtual bool tickBuild(Player* player, BuildActionIntention* bai) {
		...
		while (Multitouch::next()) {
			...
			// 正在拖摇杆的手指不参与转向 / 「按住不动 = 挖掘」判定
			if (m.pointerId == capturedPointer()) continue;
...
private:
	...
	// HUD 控件（按钮）区域，从转向区里排除掉，见 setControlAreas()
	static const int MaxControlAreas = 16;
	RectangleArea _ctrlArea[MaxControlAreas];
	int _ctrlAreaCount;

	// 被输入控件独占的指针（移动版摇杆）：转向/挖掘要跳过它
	int capturedPointer() const {
		return _holder ? _holder->getCapturedPointer() : -1;
	}
...
public:
	...
	virtual void onConfigChanged(const Config& c) {
		...
		// 把 HUD 按钮区交给转向区做排除（否则按住跳跃/升降键会开始挖方块）
		{
			RectangleArea ctrlAreas[16];
			const int n = _move.getControlAreas(ctrlAreas, 16);
			_turnBuild.setControlAreas(ctrlAreas, n);
		}
	...
	virtual bool allowPicking() {
		...
		for (int i = 0; i < pointerCount; ++i) {
			int p = pointerIds[i];
			// 摇杆正在拖的手指不参与拾取
			if (p == getCapturedPointer()) continue;
	...
	// 摇杆正在拖的那根手指、以及按在模组 HUD 按钮上的手指，都由控件独占：
	// 见 IInputHolder::getCapturedPointer
	virtual int getCapturedPointer() {
		const int joyP = _move.getJoystickPointer();
		if (joyP >= 0) return joyP;
		ModEngine* me = ModEngine::instance;
		return me ? me->hudFirstPointer() : -1;
	}
