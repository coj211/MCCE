...
struct PlayScreen: Screen
{
	...
	// 0.8.1 GUI 移植：条目元素生命周期由 GuiElementContainer 拥有（其析构会 delete children）。
	// 注意：不得再用 shared_ptr 持有同一批元素，否则容器析构 + shared_ptr 双重 delete -> 堆损坏(0xc0000374)。
	...
	// 09 · UI 覆盖系统：本屏逻辑名（mod 用 "play.<element>" 覆盖）
	const char* uiScreenId() const { return "play"; }
