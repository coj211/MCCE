...
void IngameBlockSelectionScreen::renderSlot(int32_t a2, int32_t a3, int32_t a4, float a5) {
	...
	v9 = (ItemInstance*)v8;
	if(v8) {
		ItemRenderer::renderGuiItemNew(this->minecraft->textures, v8, 0, (float)a3, (float)a4, 1.0, 1.0, 1.0);
...
void IngameBlockSelectionScreen::init() {
	...
	this->field_98.x = (float)spx - 4.0;
	this->field_98.maxX = (float)v10 + 4.0;
	this->field_98.y = (float)spy - 4.0;
	...
	if(!sel || sel->isNull()) {
		...
		if(!this->minecraft->isCreativeMode()) {
			...
			this->armorButton.y = height - this->armorButton.height;
			this->armorButton.x = 0;
