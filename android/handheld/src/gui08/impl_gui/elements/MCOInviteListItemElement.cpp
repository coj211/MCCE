...
void MCOInviteListItemElement::render(Minecraft* a2, int32_t a3, int32_t a4) {
	...
	this->fill((float)this->x, (float)this->y, (float)(this->x + this->width), (float)(this->y + 1), 0xFFCFC6BE);
	this->fill((float)this->x, (float)(this->y + 1), (float)(this->x + this->width), (float)(this->y + this->height - 1), 0xFF948681);
	this->fill((float)this->x, (float)(this->y + this->height - 1), (float)(this->x + this->width), (float)(this->y + this->height), 0xFF312D31);
	friendSearchCompleted = this->friendSearchCompleted;
	if(friendSearchCompleted == 1) {
		a2->font->drawShadow(this->field_3C, (float)this->x + 5.0, (float)this->y + (float)((this->height - 10) / 2), 0xFFFFFFFF);
	} else if(!friendSearchCompleted) {
		a2->font->draw("searching...", (float)this->x + 5.0, (float)this->y + (float)((this->height - 10) / 2), 0xFFD4C521);
	}
	this->field_40->render(a2, a3, a4);
}
void MCOInviteListItemElement::setupPositions() {
	this->field_40->x = this->x + this->width - this->field_40->width - 5;
	this->field_40->y = this->y + (this->height - this->field_40->height) / 2;
}
