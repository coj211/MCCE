...
Packet* MinecraftPackets::createPacket(int id)
{
	...
	case PACKET_ADVENTURESETTINGS:
		...
		break;
	case PACKET_SETDIMENSION:
		packet = new SetDimensionPacket();
		break;
	case PACKET_MODSLIST:
		packet = new ModListPacket();
		break;
	case PACKET_MODREQUEST:
		packet = new ModRequestPacket();
		break;
	case PACKET_MODFILE:
		packet = new ModFilePacket();
		break;
	case PACKET_MODSREADY:
		packet = new ModsReadyPacket();
		break;
	case PACKET_SETPLAYERGAMETYPE:
		packet = new SetPlayerGameTypePacket();
		break;
	case PACKET_PLAYERPOSE:
		packet = new PlayerPosePacket();
		break;
	}
