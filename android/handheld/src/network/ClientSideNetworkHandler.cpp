...
{
	// In an infinite joined world the client keeps (locally generated)
	// terrain everywhere, so any UpdateBlock may be applied directly.
...
void ClientSideNetworkHandler::handle(const RakNet::RakNetGUID& source, StartGamePacket* packet)
{
	...
	MultiPlayerLevel* level = new MultiPlayerLevel(
		...
		"temp",
		LevelSettings(packet->levelSeed, LevelSettings::validateGameType(packet->gameType), packet->worldType, packet->dimensionId),
...
// 服务器告诉我们“你的游戏模式变了”（服务器可以只给某个人开创造）。
// 就地切自己的 GameMode 实例并重算能力 —— 不用重连。
// 引擎现成方法 setIsCreativeMode() 内部就是：
//   delete gameMode; gameMode = new CreativeMode/SurvivalMode(this); initAbilities(player->abilities);
// （已确认没有别处长期缓存 gameMode 指针，删旧实例不会悬空）
void ClientSideNetworkHandler::handle(const RakNet::RakNetGUID& source, SetPlayerGameTypePacket* packet)
{
	const bool creative = (packet->gameType == GameType::Creative);
	LOGI("SetPlayerGameTypePacket: %s\n", creative ? "creative" : "survival");
#ifndef STANDALONE_SERVER
	if (minecraft->isCreativeMode() != creative)
		minecraft->setIsCreativeMode(creative);
#endif
}

// 模组（玩家姿态/动作）：某个玩家的动作变了 —— 服务器广播过来（别人的），
// 或房主/服务器自己设的。按玩家分别记下来，渲染时 HumanoidModel 每帧查。
// 这里只改本地状态，不再回发（避免环）。
void ClientSideNetworkHandler::handle(const RakNet::RakNetGUID& source, PlayerPosePacket* packet)
{
	if (ModEngine::instance)
		ModEngine::instance->applyAppearance(packet->playerId, packet->action.C_String(), packet->model.C_String());
}
...
// 
	...
	if (_mpActive) {
		int64_t key = chunkKey(packet->x, packet->z);
		_mpInFlight.erase(key);
		_mpReceived.insert(key);
		chunkReadyCheck();
		updateChunkStream();
		return;
	}
...
//
// Infinite-world rolling chunk window.
//
// For infinite joined worlds the client asks the server for the chunk
// state around the local player (the local generator only provides a
// placeholder so terrain is never missing while the authoritative chunk
// streams in). The window follows the player and fills continuously.
//
int64_t ClientSideNetworkHandler::chunkKey(int cx, int cz) {
	return ((int64_t)(uint32_t)cx << 32) | (uint32_t)cz;
}

void ClientSideNetworkHandler::resetChunkStream() {
	_mpReceived.clear();
	_mpInFlight.clear();
	_mpWindow.clear();
	_mpNextIndex = 0;
	_mpHasCenter = false;
	_mpReadySent = false;
	_mpCenterCX = _mpCenterCZ = 0;
}

void ClientSideNetworkHandler::rebuildChunkWindow(int centerCX, int centerCZ) {
	_mpWindow.clear();
	int half = MpChunkWindowHalf;
	for (int dx = -half; dx <= half; ++dx) {
		for (int dz = -half; dz <= half; ++dz) {
			IntPair c;
			c.x = centerCX + dx;
			c.y = centerCZ + dz;
			_mpWindow.push_back(c);
		}
	}
	// sort near-first (distance², then x then z) so the core is requested first
	std::sort(_mpWindow.begin(), _mpWindow.end(),
		[&](const IntPair& a, const IntPair& b) {
			int adx = a.x - centerCX, adz = a.y - centerCZ;
			int bdx = b.x - centerCX, bdz = b.y - centerCZ;
	...
	_mpNextIndex = 0;
	_mpCenterCX = centerCX;
	_mpCenterCZ = centerCZ;
	_mpHasCenter = true;
}

void ClientSideNetworkHandler::updateChunkStream() {
	if (!level || !minecraft)
		return;
	...
	int ccx = Mth::floor(p->x / (float)CHUNK_WIDTH);
	int ccz = Mth::floor(p->z / (float)CHUNK_DEPTH);
	if (!_mpHasCenter || ccx != _mpCenterCX || ccz != _mpCenterCZ) {
		rebuildChunkWindow(ccx, ccz);
	}

	// Advance the scan cursor past chunks we already have or already asked for,
	// then request up to 6 new chunks per frame.
	int budget = 6;
	while (_mpNextIndex < (int)_mpWindow.size()) {
		const IntPair& c = _mpWindow[_mpNextIndex];
		int64_t key = chunkKey(c.x, c.y);
		if (_mpReceived.count(key) == 0 && _mpInFlight.count(key) == 0) {
			if (budget <= 0)
				break;
			RequestChunkPacket packet(c.x, c.y);
			raknetInstance->send(packet);
			_mpInFlight.insert(key);
			--budget;
		}
		++_mpNextIndex;
	}

	// The cursor may have run past the end of the window while chunks are
	// still in flight; when they arrive chunkReadyCheck() re-fires this so
	// newly freed budget slots keep the window filling.
}

void ClientSideNetworkHandler::chunkReadyCheck() {
	// Signal READY once the 5x5 core around the current center has arrived.
	if (_mpReadySent || !_mpHasCenter)
		return;
	int half = MpReadyCoreHalf;
	for (int dx = -half; dx <= half; ++dx) {
		for (int dz = -half; dz <= half; ++dz) {
			if (_mpReceived.count(chunkKey(_mpCenterCX + dx, _mpCenterCZ + dz)) == 0)
				return;
		}
	}
	_mpReadySent = true;
...
void ClientSideNetworkHandler::handle(const RakNet::RakNetGUID& source, SetDimensionPacket* packet)
{
	if (!level || !minecraft || !minecraft->player)
		return;

	int curDim = level->dimension ? level->dimension->id : 0;
	LOGI("SetDimensionPacket: dim %d -> %d at %f,%f,%f\n", curDim, packet->dimensionId, packet->x, packet->y, packet->z);

	LocalPlayer* p = minecraft->player;

	if (packet->dimensionId == curDim) {
		// Same world: server-authoritative teleport.
		p->moveTo(packet->x, packet->y, packet->z, p->yRot, p->xRot);
		p->setRespawnPosition(Pos((int)packet->x, (int)packet->y, (int)packet->z));
		return;
	}

	// 1) stop the infinite-world chunk stream & drop the finite request bookkeeping
	_mpActive = false;
	resetChunkStream();
	requestNextChunkPosition = 0;
	requestNextChunkIndex = 0;
	...
	// 2) drop every entity that belongs to the old dimension world (kept the
	//    local player). They are cleaned up by the next Level::tickEntities().
	{
		EntityList list = level->entities;
		for (unsigned int i = 0; i < list.size(); ++i) {
			Entity* e = list[i];
			if (!e || e == p || e->removed)
				continue;
			if (e->isPlayer()) {
				e->reallyRemoveIfPlayer = true;
				e->remove();
			} else {
	...
	// 3) teleport & rebuild the client level for the target dimension
	p->moveTo(packet->x, packet->y, packet->z, p->yRot, p->xRot);
	p->dimension = packet->dimensionId;
	p->setRespawnPosition(Pos((int)packet->x, (int)packet->y, (int)packet->z));
	level->resetChunks(packet->dimensionId);

	if (minecraft->levelRenderer)
		minecraft->levelRenderer->allChanged();

	// 4) restart the chunk request flow for the new world
	_mpActive = level->isInfinite();
	if (_mpActive) {
		resetChunkStream();
		updateChunkStream();
...
// ===========================================================================
// Mod auto-sync during join (client side)
// ===========================================================================
void ClientSideNetworkHandler::handle(const RakNet::RakNetGUID& source, ModListPacket* packet)
{
	LOGI("ModListPacket: %d mod(s) from server\n", (int)packet->entries.size());
	_modPending.clear();
	_modTarget.clear();
	_modCurFile.clear();
	_modCurBuf.clear();
	...
	if (!minecraft || !minecraft->modEngine) {
		// No mod support on this build: nothing to sync.
		sendModsReady();
		return;
	}
	ModEngine* me = minecraft->modEngine;
	std::vector<std::string> local = me->getEnabledModFiles();
	std::vector<std::string> missing;
	for (size_t i = 0; i < packet->entries.size(); ++i) {
		const std::string& file = packet->entries[i].file;
		_modTarget.push_back(file);
		bool haveFile = false;
		for (size_t j = 0; j < local.size(); ++j)
			if (local[j] == file) { haveFile = true; break; }
		long localSize = haveFile ? me->modFileSize(file) : -1;
		if (!(haveFile && localSize == (long)packet->entries[i].size))
			missing.push_back(file);
	}

	if (missing.empty()) {
		// Already have everything the host uses -> enable that set and join.
		LOGI("ModListPacket: client already has all %d mod(s)\n", (int)packet->entries.size());
		me->applyEnabledList(_modTarget);
		sendModsReady();
		return;
	}
	...
	_modPending = missing;
	_modSyncing = true;
	LOGI("ModListPacket: requesting %d missing mod(s)\n", (int)missing.size());
	ModRequestPacket req(missing);
	raknetInstance->send(req);
#else
	sendModsReady();
#endif
}

void ClientSideNetworkHandler::handle(const RakNet::RakNetGUID& source, ModFilePacket* packet)
{
	if (!_modSyncing)
		return;

	const std::string& file = packet->file;
	if (file != _modCurFile) {
		modFinalizeCurrent();
		_modCurFile = file;
		if (packet->total < 0 || packet->total > 64 * 1024 * 1024) {
			LOGE("ModFilePacket: bad total size %d\n", packet->total);
			_modSyncing = false;
			return;
		}
		_modCurBuf.assign((size_t)packet->total, 0);
	}

	size_t start = (size_t)packet->offset;
	size_t len = packet->data.size();
	if (start + len > _modCurBuf.size()) {
		LOGE("ModFilePacket: bad offset for %s\n", file.c_str());
		_modSyncing = false;
		return;
	}
	if (len > 0)
		memcpy(_modCurBuf.data() + start, packet->data.data(), len);

	if (start + len >= _modCurBuf.size()) {
		modFinalizeCurrent();
		if (_modPending.empty())
			modAllDone();
	}
}

void ClientSideNetworkHandler::modFinalizeCurrent()
{
	if (_modCurFile.empty())
		return;
#if defined(_WIN32) || defined(__ANDROID__)
	if (minecraft && minecraft->modEngine) {
		minecraft->modEngine->writeModFileBytes(_modCurFile, _modCurBuf);
		LOGI("mod download done: %s (%d bytes)\n", _modCurFile.c_str(), (int)_modCurBuf.size());
	}
#endif
	for (size_t i = 0; i < _modPending.size(); ++i) {
		if (_modPending[i] == _modCurFile) {
			_modPending.erase(_modPending.begin() + i);
			break;
		}
	}
	_modCurFile.clear();
...
void ClientSideNetworkHandler::modAllDone()
{
	_modSyncing = false;
#if defined(_WIN32) || defined(__ANDROID__)
	if (minecraft && minecraft->modEngine) {
		// Align the enabled set (and ordering) with the host, then hot-reload
		// every mod so definitions (blocks, dimensions...) exist before the
		// world starts.
		minecraft->modEngine->applyEnabledList(_modTarget);
	}
#endif
	_modTarget.clear();
	LOGI("mod sync complete, joining...\n");
	sendModsReady();
}

void ClientSideNetworkHandler::sendModsReady()
{
	ModsReadyPacket packet;
	raknetInstance->send(packet);
}
