...
// doing any calculations.
	...
	// QPC frequency never changes during a process; cache it so getTimeS()
	// doesn't pay QueryPerformanceFrequency on every call (it is called
	// several times per frame).
	static long long _t_freq() {
...
float getTimeS() {
	...
	LARGE_INTEGER ts;
	QueryPerformanceCounter(&ts);
	return (ts.QuadPart - _t_start) / (float)_t_freq();
#else
    timeval now;
    gettimeofday(&now, 0);
