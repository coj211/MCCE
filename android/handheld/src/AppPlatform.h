...
public:
	...
	// 系统接管、不向应用派发触摸的右侧宽度（物理像素）。Android 上由窗口
	// insets 推过来：沉浸式全屏生效时是 0，导航栏/手势区露出时是它的宽度。
	// UI 布局把它当成“屏幕外”，见 Gui::uiScreenWidth()。
	virtual int getUiRightInset() { return 0; }
	...
	// 把输入法重新叫回来（不改变文本）。安卓在点击「输入框以外」的地方时会自动
	// 收起 IME —— 聊天屏里点一下画面就没法继续打字，得靠这个补回来。
	// 默认什么都不做（桌面/手柄没有软键盘）。
