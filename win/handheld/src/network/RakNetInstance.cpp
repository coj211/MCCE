...
bool RakNetInstance::connect(const char* host, int port)
{
	...
	// Pick the socket family from the host: an IPv6 literal (contains ':')
	// needs an AF_INET6 socket; everything else (IPv4 or hostname) is IPv4.
	// RakNet resolves the host against this socket's IP version, so this is
	// what enables direct IPv6 remote connections.
	bool isV6 = (host != NULL) && (strchr(host, ':') != NULL);
	socket.socketFamily = isV6 ? AF_INET6 : AF_INET;
