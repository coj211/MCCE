...
public:
	...
	static void feedText(const std::string& utf8) {
		// Append a UTF-8 byte sequence (multi-byte CJK chars etc.).
		for (size_t i = 0; i < utf8.size(); ++i)
			_inputText.push_back(utf8[i]);
	}
	static std::string getText() {
		return std::string(_inputText.begin(), _inputText.end());
	}
	// Atomically take and clear the accumulated text. Screens should use this
	// so input is consumed exactly once per frame regardless of how many
	// ticks ran or when reset() happens (fixes swallowed/lost keystrokes).
	static std::string consumeText() {
		std::string s(_inputText.begin(), _inputText.end());
		_inputText.clear();
		return s;
	}
...
private:
	...
	static bool _inited;
	// True while a text-input screen (chat box / server address box) is
	// open. The Win32 frontend uses this to enable the IME only when the
	// player actually types — otherwise a CJK input method swallows WASD/
	// inventory keys as composition keys (VK_PROCESSKEY) and gameplay keys
	// stop working.
	static bool _textInputActive;

public:
	static void setTextInputActive(bool active) { _textInputActive = active; }
	static bool isTextInputActive() { return _textInputActive; }
};
