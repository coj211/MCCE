...
void PackedScrollContainer::render(Minecraft* a2, int32_t a3, int32_t a4) {
	...
	color = vscrollbar->alpha;
	if(color > 0.0) {
		this->fill(vscrollbar->x, vscrollbar->y + 2.0, vscrollbar->x + vscrollbar->w, (float)(vscrollbar->y + vscrollbar->h) + 2.0, ((int32_t)(float)(color * 255.0) << 24) | 0xFFFFFF);
	}
	glDisable(0xC11u);
}
void PackedScrollContainer::setupPositions() {
	...
	v5 = v3 + this->y + this->field_44;
	for(auto a8: this->children) {
		a8->x = this->x + this->field_40;
		a8->width = this->width - 2 * this->field_40;
		a8->y = v5 + this->field_44;
		a8->setupPositions();
		v5 += a8->height;
	}
	v8 = v5 - v3 - this->y;
	...
	if(!v9 || v8 != v9) {
		...
		this->scrollingPane = new ScrollingPane(v18, IntRectangle{x, y, this->width, height}, IntRectangle{0, 0, this->width, (v8 < height ? height : v8) / 2}, 1, 2, Gui::GuiScale, {0, 0, 0, 0});
