...
void Touch::TouchWorldSelectionList::commit(void) {
	// 0.8.1 GUI 移植：补全 commit（0.8.1 原版为 TODO，填充世界卡片信息与贴图路径）
	for(int i = 0; i < (int)this->field_78.size(); ++i) {
		LevelSummary* v3 = &this->field_78[i];
		this->field_90.emplace_back("gui/default_world.png");
		std::vector<std::string> v12;
		v12.push_back(v3->name);
		v12.emplace_back(this->minecraft->platform()->getDateString(v3->lastPlayed));
		v12.push_back(v3->id);
		v12.emplace_back(v3->gameType == 1 ? "Creative" : (v3->gameType == 0 ? "Survival" : "Undefined"));
		this->field_84.emplace_back(v12);
	}
}
...
void Touch::TouchWorldSelectionList::selectItem(int32_t a2, bool a3) {
	if(this->selectedItem >= 0) {
...
bool Touch::TouchWorldSelectionList::isSelectedItem(int32_t a2) {
...
void Touch::TouchWorldSelectionList::renderItem(int32_t a2, int32_t a3, int32_t a4, int32_t a5, Tesselator& a6) {
	...
	if(a2 >= this->field_78.size()) {
		...
		this->minecraft->textures->loadAndBindTexture("gui/touchgui.png");
		float v22 = (float)a4;
		if(this->field_70) v18 = 0.33594;
		else v19 = 0.33594;
		a6.begin(GL_QUADS);
		a6.color(v12);
		a6.vertexUV(v9 - 27, v22, this->blitOffset, 0.65625, v18);
		a6.vertexUV(v9 - 27, v22 + 54, this->blitOffset, 0.65625, v19);
		a6.vertexUV(v9 + 27, v22 + 54, this->blitOffset, 0.86719, v19);
		a6.vertexUV(v9 + 27, v22, this->blitOffset, 0.86719, v18);
		a6.draw();
	} else {
		...
		this->minecraft->textures->loadAndBindTexture(this->field_90[a2]);
		a6.color(0.3, 1, 0.2);
		...
		int32_t v16 = v9 + 32;
		float v17 = (float)v15;
		a6.vertexUV(v17, v14, this->blitOffset, 0, 0.125);
		a6.vertexUV(v17, v14+48, this->blitOffset, 0, 0.875);
		a6.vertexUV(v16, v14+48, this->blitOffset, 1, 0.875);
		a6.vertexUV(v16, v14, this->blitOffset, 1, 0.125);
		a6.draw();
	}
