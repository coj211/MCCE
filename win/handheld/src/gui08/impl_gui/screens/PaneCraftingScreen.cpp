...
void PaneCraftingScreen::onItemSelected(int32_t a2, CItem* a3) {
	...
	if(a2 != this->field_78) {
		...
		this->scrollingPane = new ItemPane(this, this->minecraft->textures, this->field_184, a6, this->height, this->minecraft->field_20, 1);
...
void PaneCraftingScreen::render(int32_t a2, int32_t a3, float a4) {
	...
	swlast.start();
	_D6E06298->draw(Tesselator::instance, this->field_184.x - 1, this->field_184.y - 2);
	...
	float v11 = (float)((float)this->craftButton.x + (float)(v9 * 0.5)) - 8.0;
	float v12 = (float)((float)this->craftButton.y + (float)(v10 * 0.5)) - 9.0;
	ItemInstance v30;
	if(this->field_7C) {
		for(int i = 0; i < this->field_7C->field_28.size(); ++i) {
			...
			ItemRenderer::renderGuiItemNew(this->minecraft->textures, (const ItemInstance*)&v30, 0, v18, a5, 1.0, 1.0, 1.0);
		}
		...
		this->minecraft->font->drawWordWrap(this->field_80, (float)this->craftButton.x, (float)(this->craftButton.y + this->craftButton.height + 6), (float)this->craftButton.width, 0xFFF0F0F0, 1, 0);
	}
}
void PaneCraftingScreen::init() {
	...
	this->craftButton.init(this->minecraft->textures);
...
void PaneCraftingScreen::buttonClicked(Button* a2) {
	...
	if(v4 <= 0x63) {
		...
		if(v5->begin() != v5->end()) {
			this->onItemSelected(a2->id - 100, v5->at(0));
