...
void Touch::IngameBlockSelectionScreen::render(int32_t a2, int32_t a3, float a4) {
	...
	v8 = this->field_60;
	this->minecraft->textures->loadAndBindTexture("gui/itemframe.png");
	glColor4f(1.0, 1.0, 1.0, 1.0);
	this->blit(0, v8->field_228.y - 6, 0, 0, this->width, v8->field_228.height + 10, 215, 256);
...
void Touch::IngameBlockSelectionScreen::setupPositions() {
	...
	width = this->width;
	this->armorButton.y = 4;
	this->craftButton.y = 4;
	this->backButton.y = 4;
	this->backButton.x = 4;
	this->menuButton.y = 4;
	this->menuButton.x = this->backButton.x + this->backButton.width + 4;
	v2 = this->armorButton.width;
	this->selectBlocksHeader.width = width;
	v3 = width - v2;
	v4 = this->craftButton.width;
	v3 -= 4;
	this->armorButton.x = v3;
	this->craftButton.x = v3 - v4 - 4;
	minecraft = this->minecraft;
	this->selectBlocksHeader.y = 0;
	this->selectBlocksHeader.x = 0;
	this->field_178 = 0;
...
void Touch::IngameBlockSelectionScreen::buttonClicked(Button* a2) {
	...
	if(a2->id == this->menuButton.id) {
		this->minecraft->screenChooser.setScreen(PAUSE_SCREEN_0);
	}
	if(a2->id == this->craftButton.id) {
