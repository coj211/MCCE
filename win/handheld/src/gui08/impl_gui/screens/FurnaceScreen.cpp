...
void FurnaceScreen::drawSlotItemAt(Tesselator& a2, const ItemInstance* a3, int32_t a4, int32_t a5, bool_t a6) {
	...
	if(a3) {
		if(!a3->isNull()) {
			ItemRenderer::renderGuiItemNew(this->minecraft->textures, a3, 0, v6 + 7.0, v10 + 8.0, 1.0, 1.0, 1.0);
...
void FurnaceScreen::handleRenderPane(Touch::InventoryPane* a2, Tesselator& a3, int32_t a4, int32_t a5, float a6) {
	...
	if(a2) {
		...
		a2->render(a4, a5, a6);
		this->field_218->draw(a3, (float)(a2->field_228.x - 1), (float)(a2->field_228.y - 1));
	}
}
...
void FurnaceScreen::render(int32_t a2, int32_t a3, float a4) {
	...
	glDepthMask(1u);
	this->drawSlotItemAt(Tesselator::instance, this->tileEntity->getItem(this->field_F4.id), this->field_F4.x, this->field_F4.y, this->field_F4.id == this->field_200);
	this->drawSlotItemAt(Tesselator::instance, this->tileEntity->getItem(this->field_124.id), this->field_124.x, this->field_124.y, this->field_124.id == this->field_200);
	ItemInstance* v11 = this->tileEntity->getItem(this->field_154.id);
	this->drawSlotItemAt(Tesselator::instance, v11, this->field_154.x, this->field_154.y, this->field_154.id == this->field_200);
	if(!this->field_6C.isNull()) {
		if(!v11 || v11->isNull()) {
			ItemRenderer::renderGuiItemNew(this->minecraft->textures, &this->field_6C, 0, (float)this->field_154.x + 7.0, (float)this->field_154.y + 8.0, 1.0, 0.5, 1.0);
		}
		this->minecraft->font->drawWordWrap(this->field_68, (float)this->field_154.x - 24.0, (float)(this->field_154.y + this->field_154.height + 6), this->field_80, -986896, 1, 0);
	}
	this->minecraft->textures->loadAndBindTexture("gui/spritesheet.png");
	int y = this->field_154.y;
	int v15 = this->field_F4.x + 8;
	int v16 = this->tileEntity->getLitProgress(14) + 2;
	this->blit(v15, y + 8, 80, 40, 16, 16, 32, 32);
	this->blit(v15, y + 24 - v16, 112, 72 - 2 * v16, 16, v16, 32, 2 * v16);
	int v18 = this->field_F4.x + 40;
	int v19 = this->tileEntity->getBurnProgress(24);
	this->blit(v18, y + 8, 144, 40, 24, 16, 48, 32);
	this->blit(v18, y + 8, 144, 72, v19, 16, 2 * v19, 32);
}
...
void FurnaceScreen::setupPositions(){
	...
	this->field_F4.y = 36;
	this->field_124.y = height2 + 56;
	this->field_154.x = v12;
	this->field_154.y = (height2 + 92) / 2;
...
bool_t FurnaceScreen::isAllowed(int32_t a2) {
	...
	if(a2 < this->field_1F0.size()) {
		...
		v4 = this->field_200;
		if(v4 == this->field_124.id) {
			return FurnaceTileEntity::getBurnDuration(*v3) > 0;
		}
		if(v4 == this->field_F4.id) {
