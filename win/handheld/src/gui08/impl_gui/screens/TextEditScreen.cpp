...
void TextEditScreen::render(int32_t a2, int32_t a3, float a4) {
	...
	glOrtho
		...
		this->minecraft->textures->loadAndBindTexture("item/sign.png");
		...
		do {
			...
			if(v13 == v9->field_6C && Util081::utf8len(ss) <= 13) {
				std::string v20 = "";
				v20 += "> ";
				v20 += ss;
				v20 += " <";
...
void TextEditScreen::setupPositions() {
	...
	this->doneButton.x = this->width - 19;
	this->doneButton.y = 0;
}
...
void TextEditScreen::keyPressed(int32_t a2) {
	...
	if(a2 == 8) {
		...
		if(v5->textLines[currentLine].size() != 0) {
			v8 = Util081::utf8len(v5->textLines[currentLine]);
			v5->textLines[currentLine] = Util081::utf8substring(v5->textLines[currentLine], 0, v8 - 1);
			return;
		}
		v10 = currentLine - 1;
		if(v10 < 0) {
