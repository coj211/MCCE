...
var cfg = {
    ...
    shad: 0.45, sunAz: 35, sunEl: 52, vol: 0.7     // 阴影强度/太阳方位仰角/体积雾强度
};
var near = 0.05, far = 256.0;
var VER = "rc6.1-vol";   // 版本标识(便于确认加载的是新包)
...
function build() {
    ...
    var fs = compile(GL.FRAGMENT_SHADER, [
        ...
        "uniform vec3 u_sunDir;",     // 指向太阳的单位向量(与阴影光方向一致)
        "uniform vec3 u_sunC;",       // 阳光颜色(含昼夜/日出暖色)
        "uniform float u_vol;",       // 体积雾/体积光强度(0=关)
        "in vec2 vUv;",
        "out vec4 fragColor;",
        // 线性深度(窗口深度 0..1 → 视空间距离)
        ...
        "}",
        // ── 体积雾/体积光 ──
        // 便宜的流动 3D 噪声(密度扰动,呈雾团/尘埃感)
        "float volNoise(vec3 p){",
        "  return sin(p.x * 0.11 + u_time * 0.3) * sin(p.z * 0.09 - u_time * 0.2)",
        "       + 0.6 * sin(p.x * 0.37 + p.y * 0.21 + u_time * 0.5) * sin(p.z * 0.31 - p.y * 0.13 + u_time * 0.33);",
        "}",
        // 任意点是否被太阳照到(阴影图里取: 1=在阳光中 0=在影中)
        ...
        "}",
        // 沿视线步进累积散射与透射(8 段,最大 ~90m;太阳轴上更亮=丁达尔)
        "void volScatter(vec3 eye, vec3 dir, float maxD, out vec3 insc, out float trans){",
        "  insc = vec3(0.0); trans = 1.0;",
        "  if (maxD < 1.0) return;",
        "  float mD = min(maxD, 90.0);",
        "  float stepLen = mD / 8.0;",
        "  vec3 sunD = normalize(u_sunDir);",
        "  float cosT = max(dot(dir, sunD), 0.0);",
        "  float phase = 0.25 + 0.75 * cosT * cosT;",   // 朝太阳方向散射更强(光轴)
        "  for (int i = 0; i < 8; i++) {",
        "    float s = stepLen * (float(i) + 0.5);",
        "    vec3 p = eye + dir * s;",
        "    float dens = exp(-max(p.y - (eye.y - 2.0), 0.0) * 0.05);",  // 眼下方 2m 最浓,往上指数衰减
        "    dens *= 0.6 + 0.4 * volNoise(p);",
        "    float sigma = dens * 0.030 * u_vol;",
        "    float lit = pointLit(p);",
        "    insc += u_sunC * sigma * (0.20 + 1.30 * phase) * lit * stepLen * 2.0;",       // 阳光散射
        "    insc += vec3(0.55, 0.72, 1.0) * sigma * 0.10 * (1.0 - lit) * stepLen * 2.0;", // 影内极淡天光
        "    trans *= exp(-sigma * stepLen);",
        "  }",
        "}",
        ...
        "    col = clamp(col, 0.0, 1.0);",
        "  }",
        // ── 体积雾/体积光(丁达尔): 沿视线步进,阳光轴上更亮;水下/水面跳过 ──
    ...
    S.loc = {
        ...
        sunDir:  GL.glGetUniformLocation(p, "u_sunDir"),
        sunC:    GL.glGetUniformLocation(p, "u_sunC"),
        vol:     GL.glGetUniformLocation(p, "u_vol"),
...
function draw() {
    ...
    // 体积雾 uniform: 强度/太阳方向/阳光颜色(低仰角→日出日落暖橙)
    u1(L.vol, cfg.vol);
    if (L.sunDir >= 0) {
        var azr = cfg.sunAz * Math.PI / 180, elr = cfg.sunEl * Math.PI / 180;
    ...
    if (L.sunC >= 0) {
        var wrm = 1.0 - Math.min(1.0, Math.max(0.0, Math.cos(cfg.sunEl * Math.PI / 180)));
        var dF = dayFactor();
        GL.glUniform3f(L.sunC, (0.90 + 0.55 * wrm) * dF, (0.92 + 0.18 * wrm) * dF, (0.95 - 0.30 * wrm) * dF);
    }
