...
:	itemInHandRenderer(NULL)
{
	...
	assign( ER_PLAYER_RENDERER,		new PlayerRenderer(new HumanoidModel(0, 0, true), 0));
	// 模组自定义投射物（Projectile.defineProjectile）。注意分发用的是 entity->entityRendererId
	// 这个成员（不是 getEntityTypeId），所以实体构造里要把它设成这个号。
	assign( ER_MODPROJECTILE_RENDERER, new ModProjectileRenderer());
	...
	assign( ER_SCRIPTED_RENDERER,   new ScriptedMobRenderer());
