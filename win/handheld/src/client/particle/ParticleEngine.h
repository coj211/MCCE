...
public:
    ...
    void add(Particle* p);
	// Custom particles (JS-defined) with their own textures.
	void addCustom(Particle* p);
...
private:
	...
	// JS-defined particles, each with its own GL texture; rendered grouped
	// by texture in render().
	std::vector<Particle*> _customParticles;
    Textures* textures;
    Random random;
};
