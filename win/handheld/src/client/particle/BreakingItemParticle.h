...
public:
    ...
    void render(Tesselator& t, float a, float xa, float ya, float za, float xa2, float za2, int texRows) {
        float u0 = (tex % 16 + uo / 4.0f) / 16.0f;
        float u1 = u0 + 0.999f / 16.0f / 4;
        float v0 = (tex / 16 + vo / 4.0f) / (float)texRows;
        float v1 = v0 + 0.999f / (float)texRows / 4;
        float r = 0.1f * size;
