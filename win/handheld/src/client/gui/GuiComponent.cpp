...
void GuiComponent::drawRect( int x0, int y0, int x1, int y1, int color, int thickness )
{
...
void GuiComponent::drawRect( int x0, int y0, int x1, int y1, int colorTop, int colorBottom, int colorLeft, int colorRight, int thickness )
{
	fill(x0, y0, x0 + thickness, y1, colorLeft);
	fill(x1 - thickness, y0, x1, y1, colorRight);
	fill(x0, y0, x1, y0 + thickness, colorTop);
	fill(x0, y1 - thickness, x1, y1, colorBottom);
}
...
void GuiComponent::fillGradient( float x0, float y0, float x1, float y1, int col1, int col2 )
{
	...
	glDisable2(GL_TEXTURE_2D);
	glBindTexture(GL_TEXTURE_2D, 0); // belt & braces: even if a shader
	                                 // uniform still says "use texture",
	                                 // sampling an unbound texture can't
	                                 // tint this fill (fixes blue screen)
	glEnable2(GL_BLEND);
