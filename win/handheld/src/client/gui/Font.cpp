...
:	options(options),
	...
	_cjkGlyph(0), _cjkDraw(0), _latinTexture(0)
#endif
{
	init(options);
}
...
{
	// (Re)build the CJK glyph atlas via GDI. Called on every font init /
	// graphics reset, so the GL texture is always fresh.
	if (_cjkTexture) {
...
void Font::drawCjkCharImmediate(int glyphIndex, float px, float py, int r, int g, int b, int a)
{
	...
	glBegin(GL_QUADS);
	glTexCoord2f(u0, v1); glVertex2f(px, py + s);
	glTexCoord2f(u1, v1); glVertex2f(px + s, py + s);
	glTexCoord2f(u1, v0); glVertex2f(px + s, py);
