...
typedef struct ImageDef {
	...
	float width;
	float height;
	// 0.8.1 GUI 移植：子图 UV 信息（0.8.1 ImageDef 的 field_14/18/1C/20/24）
	int u;
	int v;
	int subW;
	int subH;
	...
	IntRectangle* getSrc() {
		// 0.8.1 GUI 移植：hasSubImage 时用 u/v/subW/subH 作为子图区域
		if (hasSubImage) {
			src = IntRectangle(u, v, subW, subH);
			return &src;
		}
...
public:
	...
	// 0.8.1 GUI 移植：文字渲染 Y 偏移（0.8.1 ImageButton::setYOffset）
	int yOffset;
