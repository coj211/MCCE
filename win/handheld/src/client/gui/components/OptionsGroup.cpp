...
OptionsGroup& OptionsGroup::addOptionItem( const Options::Option* option, Minecraft* minecraft ) {
	// 0.8.1 GUI 移植：NAME（玩家名）是 TextBox 输入框（0.8.1 的 Option type 2 = createTextBox）
	if(option == &Options::Option::NAME) {
		TextBox081* element = new TextBox081(minecraft, option, "");
		// 输入框给足宽度（默认 24 太窄，名字会挤在右侧看不完）
		element->width = 120;
...
void OptionsGroup::createStepSlider( const Options::Option* option, Minecraft* minecraft ) {
	...
	if (option == &Options::Option::DIFFICULTY) {
		...
		steps.push_back(0); // English
		steps.push_back(1); // 中文
	}
