...
public:
	// Persistent custom servers (added via "Add Server"): name, ip, port.
	// Saved to games/custom_servers.txt next to the executable.
	struct CustomServer { std::string name; std::string ip; int port; };
	static std::vector<CustomServer>& customServers();
	static void loadCustomServers();
	static void saveCustomServers();
	static void addCustomServer(const std::string& ip, int port);
};
